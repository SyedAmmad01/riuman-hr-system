...
class ReportController extends Controller
{
    public function daily_report(Request $request){
        ...
        ->select('consents.*' , 'candidates.name')->whereDate('consents.created_at', $date)->where('consents.status', 1)->get();
        $dailys_rejected = Consent::join('candidates', 'consents.candidate_id', '=', 'candidates.id')
        ->select('consents.*' , 'candidates.name')->whereDate('consents.created_at', $date)->where('consents.status', 0)->get();
        $data = array(
            'dailys' => $dailys,
    ...
    public function monthly_report(Request $request){
        ...
        ->select('consents.*' , 'candidates.name')->whereMonth('consents.created_at', $month)->where('consents.status', 1)->get();
        $monthlys_rejected = Consent::join('candidates', 'consents.candidate_id', '=', 'candidates.id')
        ->select('consents.*' , 'candidates.name')->whereMonth('consents.created_at', $month)->where('consents.status', 0)->get();
        $data = array(
            'monthlys' => $monthlys,
    ...
    public function yearly_report(Request $request){
        ...
        ->select('consents.*' , 'candidates.name')->whereYear('consents.created_at', $year)->where('consents.status', 1)->get();
        $yearlys_rejected = Consent::join('candidates', 'consents.candidate_id', '=', 'candidates.id')
        ->select('consents.*' , 'candidates.name')->whereYear('consents.created_at', $year)->where('consents.status', 0)->get();
        $data = array(
            'yearlys' => $yearlys,
